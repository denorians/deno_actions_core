import { eol } from "./utils.ts";

import { R_OK, W_OK } from "https://deno.land/std@0.146.0/node/fs.ts";
...
class Summary {
  ...
  private async filePath(): Promise<string> {
    ...
    const pathFromEnv = Deno.env.get(SUMMARY_ENV_VAR);
    ...
    try {
      await access(pathFromEnv, R_OK | W_OK);
    } catch {
      throw new Error(
  ...
  async clear(): Promise<Summary> {
    return await this.emptyBuffer().write({ overwrite: true });
  }
  ...
  addCodeBlock(code: string, lang?: string): Summary {
    return this.addRaw(this.wrap("pre", this.wrap("code", code), {
      ...(lang && { lang }),
    })).addEOL();
  }
      ...
      .map((row) => {
        const cells = row
          .map((cell) => {
            ...
            return this.wrap(header ? "th" : "td", data, {
              ...(colspan && { colspan }),
              ...(rowspan && { rowspan }),
            });
          })
  ...
  addImage(src: string, alt: string, options?: SummaryImageOptions): Summary {
    ...
    return this.addRaw(this.wrap("img", null, {
      src,
      alt,
      ...{
        ...(width && { width }),
  ...
  addHeading(text: string, level?: number | string): Summary {
    ...
    return this.addRaw(this.wrap(allowedTag, text)).addEOL();
  }
  ...
  addQuote(text: string, cite?: string): Summary {
    return this.addRaw(this.wrap("blockquote", text, {
      ...(cite && { cite }),
    })).addEOL();
  }
